...
classCounter = {i:0 for i in range(len(classes))}
COUNTER_TO_SEND_SYMBOL_THRESHOLD = 15 #TODO tune!
counterToSendSymbolTime = 0

#########################
# init the udp socket for sending
#ip = "192.168.205.232"
#port = 2001
...
while cv.waitKey(1) < 0:
    ...
    classIndeces = postprocess(frame, outs)
    for i in range(len(classes)):
        if i in classIndeces:
            classCounter[i] += 1
            classCounter[i] = max(classCounter[i], COUNTER_TO_SEND_SYMBOL_THRESHOLD)
        else:
            if classCounter[i] > 0:
                classCounter[i] -= 1
    if counterToSendSymbolTime == COUNTER_TO_SEND_SYMBOL_THRESHOLD:
        counterToSendSymbolTime = 0
        maxx = 0
        maxxClass = -1
        for classs in classCounter:
            if classCounter[classs] > maxx:
                maxx = classCounter[classs]
                maxxClass = classs
        if maxx > 8: # TODO tune!
            print(maxxClass, classes[maxxClass])
            #clientSocket.sendto(bytes(str(maxxClass), 'ascii'), (ip, port))

    else:
        counterToSendSymbolTime += 1
